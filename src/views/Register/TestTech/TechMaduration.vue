                ...
                <div class="info">
                    <div class="tips">
                        <div v-for="(img, idI) in imgs" :key="idI">
                            <img v-if="idI === i" id="imagenSimTest" draggable="false" :src="img.src"
                ...
                <div class="info questions">
                    ...
                    <div class="options" v-if="i === 0 || i === 22 || i === 8">
                        <div class="ThreeColumns">
                            <button v-for="(dataA, index) in data[i].answers" :key="index" @click="captureData(i, index)"
                                :class="'btnOption b' + (dataT.category_id)">{{ dataA }}</button>
                        </div>
                    </div>
                    <div class="ScaleBox" v-if="i === 1">
                        <div class="blocNums">
                            <div v-for="(dataA, index) in data[i].answers" :key="index" class="btnOptionScale b2Number"
                                @click="captureData(i, index)">
                                <small class="numberBtn">{{ dataA }}</small>
                            </div>
                        </div>
                    </div>
                    <div class="optionsBlock" v-for="(dataA, index) in data[i].answers"
                        v-if="i != 0 && i != 1 && i != 8 && i != 22">
                        <button @click="captureData(i, index)" :class="'btnOptionBlock b' + (dataT.category_id)">{{ dataA
                        }}</button>
                    </div>
                </div>
            </div>

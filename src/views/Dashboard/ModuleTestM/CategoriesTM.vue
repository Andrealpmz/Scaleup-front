    ...
    <div class="optionsInfo">
      ...
      <div class="layout elementRight">
        <div class="buttonsAlign">
          <button @click="toggleButton('list')" class="btnlist" :class="{ selected: btnListFocused === true }">
            <i class="pi pi-list" :style="{ color: svg1Color }"></i>
          </button>
          <button class="btnGrid" @click="toggleButton('grid')" :class="{ selected: btnGridFocused === true }">
      ...
      <router-link v-for="(category, i) in qData" :key="i" :to="'/CategoriesTest/' + category.nameLink">
        <div class="cardCategory">
          <div class="dataCategory">
            ...
            <div class="QuestionsInfo">
              ...
              <div class="ProgressQuestions">
                <div class="progress" :class="'cir' + (i + 1)" :style="{
          ...
          <div class="SectionBottom">
            <div v-if="category.contested != category.total" class="CategoryStatus">
              Incompleto
            </div>
            <div v-if="category.contested === category.total" class="CategoryCompleted">
              Completo
            </div>
          ...
          <tr v-for="(data, i) in qData" class="itemHover" v-bind:class="{ Zebra: i % 2 === 0 }">
            ...
            <td>
              <div class="ProgressQuestionsL">
                <div :class="'progress cir' + (i + 1)" :style="{ width: `${(data.contested / data.total) * 100}%` }">
                </div>
              </div>
            </td>
            <td>
              <div v-if="data.contested != data.total" class="CategoryStatus center">
                Incompleto
              </div>
              <div v-if="data.contested === data.total" class="CategoryCompleted center">
                Completo
              </div>
            </td>
            <td>
